...
document.body.onmousedown = function() { return false; } //so page is unselectable
	...
	var h = $("#canvas").height();
	var mx, my;
	var multiball, numBall, randBall, targetCool;
	var ballX = [];
	var ballY = [];
	var bDx = [];
	var bDy = [];
	var dist = [];
	var ballRad;
	var p1score, p2score, sPlus;
	var p1X, p2X, p1Y, p2Y, dHeight;
	var pLength, pHeight;
	var ar = [];
	var screenState;
	var pongLogo = new Image();
	var buttonColor = [];
	var paddleAI, pSpeed;
	var p1col, p2col;
	var minIndex;
	...
	*/
	...
	function paint()
	{
		...
		//Menu Screen
		if(screenState == 0){
			for(var i=0; i < 2; i++){//SP and MP buttons
		...
		if(screenState == 2){
			...
			for(var i=0; i < numBall; i++){
				...
				// Else if the ball is still travelling vertically after it it got out of the paddle, it will go towards the opponent's side.
					...
					if (ballY[i] < p1Y - 10 || ballY[i] > p1Y + pHeight +10) 
			...
			if(paddleAI){
				...
				. smoother AI movement, but still limited
				*/
